...
export default function SignupPage() {
  ...
  const onSubmit = async (values: SignupFormValues) => {
    setError(null);
    try {
      const signupRes = await signup.mutateAsync({ email: values.email, password: values.password, referralCode: values.referralCode });
  ...
  return (
    <main className={auth.card}>
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="mt-8 space-y-4" noValidate>
        ...
        <button type="submit" disabled={signup.isPending} className={auth.btnPrimary}>
