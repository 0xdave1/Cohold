...
import { useWalletBalances, useMyVirtualAccount, formatMoney } from '@/lib/hooks/use-wallet';
import { resolveDedicatedAccountPillState } from '@/lib/wallet/dedicated-account-display';
...
export default function HomeDashboardPage() {
 ...
 // const { data: onboardingChecklist, isLoading: checklistLoading, isError: checklistError } = useOnboardingChecklist();
  ...
  const virtualAccountQuery = useMyVirtualAccount();
  ...
  const vaStatus =
    virtualAccountQuery.data?.status ??
    (dashboardSummary?.virtualAccount?.status as string | undefined) ??
    'UNKNOWN';
  const dedicatedAccountPill = resolveDedicatedAccountPillState({
    kycVerified: isVerified,
    isLoading: virtualAccountQuery.isLoading && !virtualAccountQuery.data,
    status: vaStatus,
    accountNumberLast4:
      dashboardSummary?.virtualAccount?.accountNumberLast4 ??
      virtualAccountQuery.data?.accountNumberLast4,
    accountNumber:
      virtualAccountQuery.data?.status === 'ACTIVE' ? virtualAccountQuery.data.accountNumber : null,
  });

  const handleDedicatedAccountPillClick = () => {
    if (!isVerified) return;
    if (dedicatedAccountPill.kind === 'active') {
      setShowAccountDetailsModal(true);
      return;
    }
    if (dedicatedAccountPill.kind === 'retry') {
  ...
  return (
    <div className="space-y-6">
      ...
      <WalletBalanceCard
        ...
        dedicatedAccountPill={dedicatedAccountPill}
        onDedicatedAccountPillClick={handleDedicatedAccountPillClick}
        onToggleBalanceVisible={() => setBalanceVisible((v) => !v)}
          ...
          showPendingWithdrawal ? (
            ...
            >
              <span className="truncate">
                ...
                {pendingWd!.totalNetAmount ? ` · ${formatMoney(pendingWd!.totalNetAmount, 'NGN')} net` : null}
              </span>
            </Link>
          ) : null
        }
      ...
      {showAccountDetailsModal && (
        <AccountDetailsModal
          virtualAccount={
            virtualAccountQuery.data?.status === 'ACTIVE' ? virtualAccountQuery.data : undefined
          }
