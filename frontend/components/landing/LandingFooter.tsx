...
const FOOTER_COLUMNS = {
  ...
  company: [
    ...
    { href: 'mailto:careers@cohold.com', label: 'Careers' },
              ...
              {FOOTER_COLUMNS.company.map((l) => (
                <li key={l.label}>
                  {l.href.startsWith('mailto:') ? (
                    <a href={l.href} className="text-sm text-white/85 hover:text-white transition-colors">
                      {l.label}
                    </a>
                  ) : (
                    <Link href={l.href} className="text-sm text-white/85 hover:text-white transition-colors">
          ...
          <div className="flex gap-3">
            {SOCIAL.map((s) => (
              <a
                ...
                className="flex h-10 w-10 items-center justify-center rounded-full border border-white/20 text-sm font-semibold text-white/90 hover:border-[#E8AB3E]/60 hover:text-[#E8AB3E] transition-colors"
                aria-label={s.label}
              >
                {s.icon}
              </a>
