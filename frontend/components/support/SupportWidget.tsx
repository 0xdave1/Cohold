...
export function SupportWidget() {
  ...
  useSupportSocket(open ? conversationId : null);

  const { data: messagesData } = useSupportMessages(open && mode === 'thread' ? conversationId : null, 1, 100);
  ...
  const submitTriage = async () => {
    ...
    try {
      ...
      // optional attachment: presign + upload then include on message
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-20 right-4 z-50 inline-flex h-12 items-center gap-2 rounded-full bg-cohold-blue px-4 text-sm font-semibold text-white shadow-lg md:bottom-6"
      ...
      {open ? (
        <div className="fixed bottom-36 right-4 z-50 w-[min(92vw,420px)] overflow-hidden rounded-2xl border border-dashboard-border bg-white shadow-2xl md:bottom-24">
                      ...
                      >
                        ...
                        {m.attachments && m.attachments.length > 0 ? (
                          <p className="mt-1 text-[10px] opacity-90">
                            Attachment saved ({m.attachments.length})
                          </p>
                        ) : null}
                      </div>
                    </div>
