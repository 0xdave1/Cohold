...
export class StorageService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    const accessKey = this.configService.get<string>('config.s3.accessKeyId');
    const secretKey = this.configService.get<string>('config.s3.secretAccessKey');
    const endpoint = this.configService.get<string>('config.s3.endpoint');
    const region = this.configService.get<string>('config.s3.region');
    this.bucket = this.configService.get<string>('config.s3.bucket') ?? 'cohold-assets';
